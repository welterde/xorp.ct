...
def parse_args(file, lineno, str):
    ...
    for t in toks:
        ...
        if type == 'list':
            ...
            if member_type not in xrl_atom_type:
...
def fix_up_line(line):
    # chomp whitespace
    #table = str.maketrans("","")
    #line =  str.translate(line, table, string.whitespace)
                        ...
                        for tif in target_interfaces:
                            ...
                            for i in self._interfaces:
                                if tif_name == i.name():
                                    found = 1
                            if found == 0:
                                print("Interface %s/%s not defined" % tif, "in file %s line %d\n" % (get_input_file(), get_input_line()))
